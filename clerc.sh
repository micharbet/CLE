...
#: placeholder for now, otherwise this will use variables CLE_BG and _C0
_clebg() {
	[ "$CLE_BG" ] || return #: do nothing if backround color is undefined
	_C0=$'\e]11;'$CLE_BG$'\e\\'
}
...
_CT=$'\e]0;'; _Ct=$'\007' #: generic window title escapes
case $TERM in
linux) CLE_PT='';; #: no tits on text console
screen*)
	printf "$_CT screen: $CLE_USER@$CLE_SHN$_Ct" #: set main screen  title
	_ST='\u'                                     #: titles inside the screen should be short
	_CT=$'\ek'; _Ct=$'\e\\';; #: screen's specific codes to set screen title
